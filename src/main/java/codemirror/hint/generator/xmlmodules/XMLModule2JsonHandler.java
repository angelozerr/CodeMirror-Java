  ...
  public void startElement(String uri, String localName, String name, Attributes attributes) throws SAXException {
    try {
      if ((MODULE_ELT).equals(name)) {
        ...
        currentParamName = attributes.getValue(FUNCTION_PARAM_NAME_ATTR);
        if (doc.length() > 0) {
        ...
        this.parameters.add(new Parameter(currentParamName, attributes.getValue(AS_ATTR)));
  ...
  public void endElement(String uri, String localName, String name) throws SAXException {
    try {
      if ((MODULE_ELT).equals(name)) {
        ...
        handler.addFunction(funcName, parameters, returnType, doc.toString());
        this.functionParsing = false;
        this.doc.setLength(0);
      } else if (FUNCTION_PARAM_ELT.equals(name) && functionParsing) {
        if (doc.length() > 0) {
